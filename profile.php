...
$items = $conn->osc_dbFetchResults("SELECT * FROM %st_item i, %st_item si WHERE i.fk_i_user_id = %d AND si.fk_i_item_id = i.pk_i_id AND si.i_amount > 0 LIMIT 5", DB_TABLE_PREFIX, DB_TABLE_PREFIX, Params::getParam('user_id'));
$items = Item::newInstance()->extendData($items);
View::newInstance()->_exportVariableToView('items', $items);
?>
<h1><?php echo sprintf(__('Welcome to my Shop : %s', 'shop'), osc_user_name()); ?></h1>
...
<div style="width:30%; float:left;">
    ...
    <ul>
        ...
        <li><a href="<?php echo osc_render_file_url(osc_plugin_folder(__FILE__)."contact.php&user_id=".osc_user_id()); ?>" ><?php _e("Contact seller", "shop"); ?></a></li>
    </ul>
</div>
